...
Pagina* criaPagina(int ordem){
	...
	if(nova == NULL){
		perror("Erro ao alocar página");
		return NULL;
	}

    nova->n = 0;
	
	nova->registro = (Item_arq*)malloc((2*ordem)*sizeof(Item_arq));
	if(nova->registro == NULL){
        perror("Erro ao alocar registro");
        free(nova);
		return NULL;
    }

	nova->apontador = (Pagina**)malloc(((2*ordem)+1)*sizeof(Pagina*));
	if(nova->apontador == NULL){
        perror("Erro ao alocar apontador");
        free(nova->registro);
        free(nova);
		return NULL;
    }
...
void ins_arvoreB(Item_arq reg, Pagina* pag_atual, bool* cresceu, Item_arq* reg_retorno, Pagina** pont_retorno, int ordem){ // MODIFICADO: int* cresceu -> bool* cresceu; Pagina* pont_retorno -> Pagina** pont_retorno
	long int i = 1;
	long int j;
	...
	ins_arvoreB(reg, pag_atual->apontador[i], cresceu, reg_retorno, pont_retorno, ordem);
	...
	if(pag_atual->n < (ordem * 2))
	{
		...
		*cresceu = false; // MODIFICADO: false em vez de 	
		return;
	}

	// Não há verificação de NULL no original para este malloc, mantido assim.
	Pagina* pont_temporario = criaPagina(ordem);
	if (!pont_temporario) return;
	pont_temporario->n = 0;
...
void insere_arvoreB(Item_arq reg, Pagina** arvore, int ordem){ // MODIFICADO: Aceita Pagina**
	bool cresceu; // MODIFICADO: de int para bool
	Item_arq reg_retorno;
	Pagina* pont_retorno = NULL; // Ponteiro simples aqui, seu endereço é passado para ins_arvoreB

	Pagina* pont_temporario = criaPagina(ordem);
	if(pont_temporario == NULL) return;

	ins_arvoreB(reg, *arvore, &cresceu, &reg_retorno, &pont_retorno, ordem); // MODIFICADO: passa &pont_retorno
	...
	if(cresceu) // MODIFICADO: usar bool
	{
		...
		*arvore = pont_temporario; // MODIFICADO: *arvore para atualizar a raiz corretamente.
  		...
  		libera_arvoreB(pont_temporario); 
	}
}
...
void libera_arvoreB(Pagina* pagina) {
    ...
    if (pagina->apontador != NULL && pagina->n >= 0) {
